...
def process_and_store_pdf(
    ...
    """
    Process a PDF file and store it in Pinecone as multiple document chunks.
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            metadata = {
                ...
                "document_type": "company_faq"  # Store as company FAQ to make it searchable
            }
            
            # Store the chunk
            stored_id = upsert_faq_document(
                text=chunk,
                doc_id=chunk_id,
                metadata=metadata
            )
        ...
        logger.info(f"Successfully processed and stored PDF: {filename} ({len(chunks)} chunks)")
        return result
...
def delete_pdf_document(base_doc_id: str) -> Dict[str, Any]:
    """
    Delete all chunks of a PDF document from Pinecone.
    
    Args:
        base_doc_id: Base document ID to delete all chunks for
    ...
    try:
        ...
        chunk_ids = [match.id for match in response.matches or []]
        
        if not chunk_ids:
            return {
                "success": False,
                "message": f"No chunks found for document {base_doc_id}",
                "deleted_chunks": 0
            }
        ...
        logger.info(f"Successfully deleted PDF document {base_doc_id} ({len(chunk_ids)} chunks)")
        return result
        
    except Exception as e:
        logger.error(f"Error deleting PDF document {base_doc_id}: {e}")
        return {
            "success": False,
            "error": str(e),
            "deleted_chunks": 0
